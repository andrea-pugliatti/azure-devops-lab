...
Il budget creato `budget-cea-d2109c` è associato al Resource Group, ha un importo di 150 euro e un periodo di ripristino mensile e la soglia dell'80% del costo effettivo. Il destinatario inserito è l'email del team FinOps.
Il lock creato è di tipo `CanNotDelete`. Si può controllare la presenza del Lock tramite il comando `az lock list --resource-group "$LAB_RG" --output table`. Si può verificare che la lettura sia consentita tramite `az group show --name "$LAB_RG" --output table`. Si può verificare che l'operazione di eliminazione del resource group non è consentita con il comando `az group delete --name "$LAB_RG" --yes --no-wait`.
...
Verifica: Verificare quale utente sta eseguendo il comando con il comando `az account show --query user.name --output tsv` e verificare ruoli correnti possiede l'utente sullo scope con il comando `az role assignment list --scope "$RG_SCOPE" --include-inherited --output table`.
...
Verifica: Individuare il lock applicato con il comando `az lock list --resource-group "$LAB_RG" --output table`.
Correzione: Rimuovere il lock prima di lanciare la cancellazione con il comando `az lock delete --name "<NOME_LOCK>" --resource-group "$LAB_RG"`. Una volta rimosso il vincolo, procedere con l'eliminazione della risorsa o del resource group.
...
```bash
LOCK_NAME=$(az lock list --resource-group "$LAB_RG" --query "[0].name" --output tsv)
az lock delete --name "$LOCK_NAME" --resource-group "$LAB_RG"
```
